...
Table~\ref{tab:per-class} reports the per-class F1 scores for both methods.

\begin{table}[htbp]
\centering
\caption{Per-class F1 scores on SDI-4 test (seed 42).}
\label{tab:per-class}
\begin{tabular}{l C{2.2cm} C{2.2cm}}
\toprule
\textbf{Class} & \textbf{CE Baseline} & \textbf{Proposed} \\
\midrule
Healthy  & --- & --- \\
BG       & --- & --- \\
WSSV     & --- & --- \\
...
Detailed per-class metrics are available from the retained confusion matrices in the experiment logs. The table above will be populated from the machine-readable classification reports when the corresponding evidence archive is fully extracted.
...
The FP32 model size of the proposed classifier is approximately 39.52 MiB (TFLite format), as discussed in Section~\ref{sec:mobile}.