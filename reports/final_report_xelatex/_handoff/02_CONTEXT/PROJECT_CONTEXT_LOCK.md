...
The immediate task is to create a stable, correctly formatted Final Report baseline.
Do NOT apply Council revision blue (`#1F4E79`) during this formatting-only rebuild unless the user explicitly starts the Council-content revision phase.
Use normal black report text for this baseline.

## Latest local evidence files visible in D:\CVio
The local audit inventory includes:
- `CVio_YOLO26m_vs_YOLO26x_TrainVal_FINAL.zip`
- `CVio_Final_Proposed_T4_Seed42_FINAL.zip`
- `CVio_NoTrain_EvidenceRepair_FINAL.zip`
- `timm_baseline_comparison_seed42.csv`
- `yolo_baseline_comparison_seed42.csv`
- `CVio_Classification_Kaggle_T4x2_Evidence_2026-08-11.zip`

These may contain newer evidence than Report7.
Do not silently replace report values from these archives.
First trace provenance, write an evidence-change note, and only integrate values that are internally consistent and explicitly supported.
