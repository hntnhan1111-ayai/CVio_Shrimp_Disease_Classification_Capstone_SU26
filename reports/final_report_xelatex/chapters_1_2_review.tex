...
\usepackage{geometry}
\geometry{left=2.15cm,right=2.15cm,top=1.6cm,bottom=1.8cm}
\usepackage{xcolor,colortbl,graphicx,float,longtable,array,ragged2e,caption,titlesec,fancyhdr,enumitem,microtype,hyperref,framed}
\definecolor{RevisionYellow}{RGB}{255,255,0}
\definecolor{HeaderPeach}{RGB}{249,218,199}
\definecolor{ChapterRed}{RGB}{193,0,0}
\colorlet{shadecolor}{RevisionYellow}
...
\titleformat{\chapter}[hang]{\bfseries\color{ChapterRed}\fontsize{18}{22}\selectfont}{\thechapter.}{0.55em}{}
\titlespacing*{\chapter}{0pt}{0pt}{12pt}
\titleformat{\section}{\bfseries\fontsize{15}{18}\selectfont}{\thesection.}{0.5em}{}
\titleformat{\subsection}{\bfseries\fontsize{13.5}{16}\selectfont}{\thesubsection}{0.5em}{}
...
\linespread{1.18}
\setlist{leftmargin=1.1cm,itemsep=4pt,topsep=4pt}
\pagestyle{fancy}
\fancyhf{}
\fancyfoot[C]{\thepage}
...
\begin{longtable}{|>{\centering\arraybackslash}p{4.10cm}|>{\centering\arraybackslash}p{11.70cm}|}
\caption{Project information}\\
\hline
\rowcolor{HeaderPeach}\textbf{Item} & \textbf{Information} \\ \hline
\endfirsthead
...
\hline
\endlastfoot
\rowcolor{RevisionYellow}English project name & Lightweight Deep Learning for Robust Shrimp Disease Detection on Mobile Devices under Diverse Environmental Conditions \\ \hline
\rowcolor{RevisionYellow}Vietnamese project name & Phát hiện bệnh tôm bằng mô hình học sâu nhẹtrên thiết bị di động trong nhiều điều kiện môi trường khác nhau \\ \hline
\rowcolor{RevisionYellow}Project code & AIP491 01 \_ \\ \hline
\rowcolor{RevisionYellow}Group name & CVio \\ \hline
\rowcolor{RevisionYellow}Academic program & Bachelor of Artificial Intelligence \\ \hline
\rowcolor{RevisionYellow}Department & Department of Artificial Intelligence \\ \hline
\rowcolor{RevisionYellow}Institution & FPT University - Campus Can Tho \\ \hline
\rowcolor{RevisionYellow}Project type & AI-based four-class image classification, BG/WSSV instance segmentation, robustness and explainability evaluation, deployment-oriented lightweight-model research, native Android application development, and Firebase-backed data integration \\ \hline
\rowcolor{RevisionYellow}Software type & Native Android mobile application with integrated artificial-intelligence and data-management functions \\ \hline
\rowcolor{RevisionYellow}Target platform & Android application using a conditional two-model TensorFlow Lite/LiteRT cascade on the common CPU execution path, with Firebase data services \\ \hline
\rowcolor{RevisionYellow}Main AI tasks & Four-class shrimp disease image classification and BG/WSSV disease-region instance segmentation, with Healthy retained as an empty-label segmentation negative \\ \hline
\rowcolor{RevisionYellow}Main technologies & Python, PyTorch, TorchVision, Ultralytics, OpenCV, Jupyter/Kaggle Notebook, TensorFlow Lite/LiteRT, Kotlin, Android Studio, Firebase, Git, and GitHub \\ \hline
\end{longtable}
\endgroup
\begin{revisionblock}
...
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{1.16}
\begin{longtable}{|>{\centering\arraybackslash}p{4.30cm}|>{\centering\arraybackslash}p{5.10cm}|>{\centering\arraybackslash}p{2.80cm}|>{\centering\arraybackslash}p{3.60cm}|}
\caption{Supervisor information}\\
\hline
\rowcolor{HeaderPeach}\textbf{Full Name} & \textbf{Email} & \textbf{Mobile} & \textbf{Title} \\ \hline
\endfirsthead
...
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{1.16}
\begin{longtable}{|>{\centering\arraybackslash}p{4.20cm}|>{\centering\arraybackslash}p{5.00cm}|>{\centering\arraybackslash}p{2.80cm}|>{\centering\arraybackslash}p{3.80cm}|}
\caption{Team member information}\\
\hline
\rowcolor{HeaderPeach}\textbf{Full Name} & \textbf{Email} & \textbf{Mobile} & \textbf{Role} \\ \hline
\endfirsthead
...
\subsection{\texorpdfstring{\colorbox{RevisionYellow}{Project Overview}}{Project Overview}}
\begin{revisionblock}
...
\end{revisionblock}
\section{\texorpdfstring{\colorbox{RevisionYellow}{Project Background}}{Project Background}}
\begin{revisionblock}
...
\end{revisionblock}
\section{\texorpdfstring{\colorbox{RevisionYellow}{Project Objective}}{Project Objective}}
\begin{revisionblock}
...
\item \colorbox{RevisionYellow}{\parbox{0.91\linewidth}{Benchmark representative TIMM and official Ultralytics YOLO classification models and justify the selected deployment-oriented classifier under a preferred budget of approximately 15 million parameters or fewer; larger models may be retained as higher-capacity diagnostic controls rather than eligible final mobile candidates.}}
\item \colorbox{RevisionYellow}{\parbox{0.91\linewidth}{Improve the selected YOLO26m-cls backbone using ASL-LDAM and SimAM-DCFR, with Macro-F1 as the primary classification metric.}}
...
\item \colorbox{RevisionYellow}{\parbox{0.91\linewidth}{Apply specimen-grouped splitting where specimen identity is recoverable, quantify leakage-prone image-level alternatives, and document the grouping boundary for unmatched-name additions whose filenames do not expose compatible specimen identities.}}
\item Evaluate attention-based and Fourier-domain segmentation improvements against controlled baselines.
\item \colorbox{RevisionYellow}{\parbox{0.91\linewidth}{Convert and integrate the final classification and conditional instance-segmentation artifacts into one native Android inference cascade using TensorFlow Lite/LiteRT, and assess branch-aware operational runtime on real Android devices.}}
...
\end{enumerate}
\section{\texorpdfstring{\colorbox{RevisionYellow}{Problem Statement}}{Problem Statement}}
\begin{revisionblock}
...
\end{revisionblock}
\section{\texorpdfstring{\colorbox{RevisionYellow}{Significance of the Project}}{Significance of the Project}}
\begin{revisionblock}
...
\end{revisionblock}
\section{\texorpdfstring{\colorbox{RevisionYellow}{Project Scope and Limitations}}{Project Scope and Limitations}}
\begin{revisionblock}
...
\end{revisionblock}
\subsection{\texorpdfstring{\colorbox{RevisionYellow}{Project Scope}}{Project Scope}}
\begin{revisionblock}
...
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{1.16}
\begin{longtable}{|>{\centering\arraybackslash}p{3.10cm}|>{\centering\arraybackslash}p{6.00cm}|>{\centering\arraybackslash}p{6.70cm}|}
\caption{Final project scope and explicit limitations}\\
\hline
\rowcolor{HeaderPeach}\textbf{Scope component} & \textbf{Included work} & \textbf{Boundary / limitation} \\ \hline
\endfirsthead
...
\hline
\endlastfoot
\rowcolor{RevisionYellow}Classification & Predict Healthy, BG, WSSV, and WSSV BG from shrimp images; \_ Macro-F1 is the principal ranking metric. & Evidence is limited to the documented datasets, source regimes, splits, and accepted checkpoints; it does not establish universal field generalization. \\ \hline
\rowcolor{RevisionYellow}Instance segmentation & Predict BG and WSSV disease-region masks; retain Healthy images as empty-label negatives. & Masks were produced by a single annotator without independent expert review. Specimen grouping is enforced only where identity is recoverable; unmatched-name additions cannot be claimed as specimen-grouped. \\ \hline
\rowcolor{RevisionYellow}Robustness & Evaluate task-appropriate synthetic noise, blur, lighting, contrast, and related degradation conditions. & The finite corruption set does not represent every farm, camera, water condition, or real acquisition disturbance. \\ \hline
\rowcolor{RevisionYellow}Explainability & Use heatmap-based classification XAI and diagnostic attention/Fourier analyses. & These analyses support qualitative or mechanistic interpretation under recorded protocols; they do not establish causal, clinical, or mask-accurate evidence. \\ \hline
\rowcolor{RevisionYellow}Mobile application & Provide a native Android classifier-to-conditional-segmentation cascade with user/admin workflows and Firebase services. & Android only; no iOS validation. Application-recorded timing may include non-model overhead and is not guaranteed real-time performance on all devices. \\ \hline
\rowcolor{RevisionYellow}Additional-source / integrated classification & Use EXT-3-Original as a separate three-class retraining regime and SDI-4 + EXT-3-Original as partition-wise integrated four-class retraining. & The integrated regime is not zero-shot external inference and is affected by source composition and class-prior differences. \\ \hline
\rowcolor{RevisionYellow}Real-world operational evidence & Use 21 usable photographs collected across three member phone cameras and record 21 application runs per Android device. & Photographs lack veterinary/laboratory ground truth; image identity/order was not standardized across devices, so the study is operational runtime evidence rather than field accuracy or strict hardware ranking. \\ \hline
\rowcolor{RevisionYellow}Intended use & Provide preliminary image-based screening, localized visual evidence, and reproducible research demonstration. & The system does not replace laboratory testing, veterinary assessment, treatment prescription, or autonomous clinical decisions. \\ \hline
\end{longtable}
\endgroup
\begin{revisionblock}
...
\end{revisionblock}
\subsection{\texorpdfstring{\colorbox{RevisionYellow}{Project Limitations}}{Project Limitations}}
\begin{revisionblock}
...
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{1.16}
\begin{longtable}{|>{\centering\arraybackslash}p{3.00cm}|>{\centering\arraybackslash}p{2.20cm}|>{\centering\arraybackslash}p{2.70cm}|>{\centering\arraybackslash}p{7.60cm}|}
\caption{Team structure and roles}\\
\hline
\rowcolor{HeaderPeach}\textbf{Member} & \textbf{Student ID} & \textbf{Role} & \textbf{Main responsibilities} \\ \hline
\endfirsthead
...
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{1.16}
\begin{longtable}{|>{\centering\arraybackslash}p{4.00cm}|>{\centering\arraybackslash}p{11.80cm}|}
\caption{Roles and responsibilities}\\
\hline
\rowcolor{HeaderPeach}\textbf{Role} & \textbf{Responsibility} \\ \hline
\endfirsthead
...
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{1.16}
\begin{longtable}{|>{\centering\arraybackslash}p{3.60cm}|>{\centering\arraybackslash}p{7.20cm}|>{\centering\arraybackslash}p{2.10cm}|>{\centering\arraybackslash}p{2.90cm}|}
\caption{Project communication plan}\\
\hline
\rowcolor{HeaderPeach}\textbf{Communication item} & \textbf{Participants and purpose} & \textbf{Frequency} & \textbf{Tool / method} \\ \hline
\endfirsthead
...
\hline
\rowcolor{HeaderPeach}\textbf{CRISP-DM phase} & \textbf{project application} \\ \hline
\endfirsthead
...
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{1.16}
\begin{longtable}{|>{\centering\arraybackslash}p{1.20cm}|>{\centering\arraybackslash}p{9.30cm}|>{\centering\arraybackslash}p{2.60cm}|>{\centering\arraybackslash}p{2.40cm}|}
\caption{Detailed Work Breakdown Structure and estimated effort}\\
\hline
\rowcolor{HeaderPeach}\textbf{\#} & \textbf{WBS Item} & \textbf{Complexity} & \textbf{Est. effort (man-days)} \\ \hline
\endfirsthead
...
\rowcolor{RevisionYellow}5.6 & Retain Healthy images as empty-label negatives & Simple & 2 \\ \hline
...
\rowcolor{RevisionYellow}5.10 & Create grouped-specimen split & Complex & 6 \\ \hline
...
\rowcolor{RevisionYellow}6.4 & Select grouped-split segmentation baseline & Medium & 3 \\ \hline
...
\rowcolor{RevisionYellow}6.9 & Review Fourier-domain augmentation methods & Complex & 5 \\ \hline
\rowcolor{RevisionYellow}6.10 & Implement Fourier transformation variants & Complex & 8 \\ \hline
\rowcolor{RevisionYellow}6.11 & Run Fourier segmentation experiments & Complex & 9 \\ \hline
...
\rowcolor{RevisionYellow}7.5 & Package final two-model TFLite bundle & Complex & 7 \\ \hline
...
\rowcolor{RevisionYellow}7.12 & Capture three-device runtime evidence & Medium & 4 \\ \hline
...
\rowcolor{RevisionYellow}8.10 & Submit initial defense DOCX package & Simple & 2 \\ \hline
...
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{1.16}
\begin{longtable}{|>{\centering\arraybackslash}p{4.50cm}|>{\centering\arraybackslash}p{2.60cm}|>{\centering\arraybackslash}p{2.60cm}|>{\centering\arraybackslash}p{2.60cm}|>{\centering\arraybackslash}p{2.60cm}|}
\caption{Responsibility assignments}\\
\hline
\rowcolor{HeaderPeach}\textbf{Responsibility} & \textbf{Tran Huu Nhan} & \textbf{Le Thi Huynh Nhu} & \textbf{Nguyen Van Phong} & \textbf{Nguyen Dinh Vinh} \\ \hline
\endfirsthead
...
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{1.16}
\begin{longtable}{|>{\centering\arraybackslash}p{1.00cm}|>{\centering\arraybackslash}p{4.80cm}|>{\centering\arraybackslash}p{3.40cm}|>{\centering\arraybackslash}p{6.20cm}|}
\caption{Project risks and response plans}\\
\hline
\rowcolor{HeaderPeach}\textbf{\#} & \textbf{Risk description} & \textbf{Impact / likelihood} & \textbf{Response plan} \\ \hline
\endfirsthead
...
\hline
\endlastfoot
\rowcolor{RevisionYellow}1 & Repeated-specimen leakage & Impact: Critical; likelihood: Medium & Group by specimen and audit overlap where filenames expose IDs; report unmatched-name additions separately because specimen identity is unavailable. \\ \hline
\rowcolor{RevisionYellow}2 & Single-annotator mask inconsistency & Impact: High; likelihood: Medium & Use one documented labeling policy for consistency; disclose the absence of independent expert review as a limitation. \\ \hline
...
\rowcolor{RevisionYellow}4 & Inconsistent baseline settings & Impact: Critical; likelihood: Medium & Fix dataset split, seed, preprocessing, checkpoint selection, metrics, class order, and evaluation scripts for direct comparisons. \\ \hline
...
\rowcolor{RevisionYellow}10 & Scope creep after review feedback & Impact: High; likelihood: Medium & Apply the change-management process; assess schedule and dependencies; update WBS and meeting minutes. \\ \hline
\rowcolor{RevisionYellow}11 & Overclaiming diagnosis or generalization & Impact: Critical; likelihood: Medium & Describe the system as preliminary screening; state dataset, annotation, corruption, mobile, and external-validation limitations explicitly. \\ \hline
\rowcolor{RevisionYellow}12 & Final document inconsistency & Impact: High; likelihood: Medium & Review before 5 August, then complete the Council-mandated cross-document audit and revised submission before 12:00 on 15 August 2026. \\ \hline
\end{longtable}
\endgroup
...
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{1.16}
\begin{longtable}{|>{\centering\arraybackslash}p{3.20cm}|>{\centering\arraybackslash}p{8.10cm}|>{\centering\arraybackslash}p{4.50cm}|}
\caption{Quality management plan}\\
\hline
\rowcolor{HeaderPeach}\textbf{Quality area} & \textbf{Control activities} & \textbf{Acceptance output} \\ \hline
\endfirsthead
...
\hline
\endlastfoot
\rowcolor{RevisionYellow}Dataset integrity & Verify counts, class mapping, duplicate handling, specimen groups, split overlap, mask format, and representative samples. & Audited manifests, statistics, and visual examples \\ \hline
...
\rowcolor{RevisionYellow}Segmentation experiments & Use documented mask semantics, grouped split, healthy negatives, mask mAP metrics, qualitative predictions, and leakage analysis. & Comparable grouped-split segmentation evidence \\ \hline
\rowcolor{RevisionYellow}Model improvement & Require improvement over the relevant baseline on the declared primary metric and provide component-level evidence without mixing incompatible runs. & Supported proposed-method claim \\ \hline
\rowcolor{RevisionYellow}Mobile application & Verify authentication, camera/ gallery input, preprocessing, TFLite inference, result display, mask display, history, export/delete, Firebase, and administrator functions. & Functional end-to-end prototype \\ \hline
...
\rowcolor{RevisionYellow}Document quality & Use consistent section numbering, table format, figure placement, captions, terminology, citations, metric values, and limitations. & Submission-ready final DOCX \\ \hline
\end{longtable}
\endgroup
...
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{1.16}
\begin{longtable}{|>{\centering\arraybackslash}p{3.70cm}|>{\centering\arraybackslash}p{7.70cm}|>{\centering\arraybackslash}p{4.40cm}|}
\caption{Project resources and funding status}\\
\hline
\rowcolor{HeaderPeach}\textbf{Resource category} & \textbf{Resource / funding source} & \textbf{Recorded cost status} \\ \hline
\endfirsthead
...
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{1.16}
\begin{longtable}{|>{\centering\arraybackslash}p{4.00cm}|>{\centering\arraybackslash}p{5.70cm}|>{\centering\arraybackslash}p{6.10cm}|}
\caption{Change categories and control requirements}\\
\hline
\rowcolor{HeaderPeach}\textbf{Change category} & \textbf{Assessment criterion} & \textbf{Required control} \\ \hline
\endfirsthead
...
\end{revisionblock}
\subsubsection{\texorpdfstring{\colorbox{RevisionYellow}{Post-Defense Council Revision Cycle}}{Post-Defense Council Revision Cycle}}
\begin{revisionblock}
...
\end{revisionblock}
\subsection{\texorpdfstring{\colorbox{RevisionYellow}{Closure and Evaluation}}{Closure and Evaluation}}
\begin{revisionblock}
...
\setlength{\tabcolsep}{2pt}
\renewcommand{\arraystretch}{1.16}
\begin{longtable}{|>{\centering\arraybackslash}p{4.40cm}|>{\centering\arraybackslash}p{11.40cm}|}
\caption{Project closure and Definition of Done}\\
\hline
\rowcolor{HeaderPeach}\textbf{Closure criterion} & \textbf{Definition of Done} \\ \hline
\endfirsthead
...
\rowcolor{RevisionYellow}Segmentation research complete & BG/WSSV labels, Healthy empty-label handling, the specimen-grouped 1,149-image subset, the unmatched-name expansion boundary, baselines, attention/Fourier evidence, metrics, qualitative analysis, and limitations are documented. \\ \hline
...
\rowcolor{RevisionYellow}Mobile prototype complete & One common Android application/model configuration executes the FP32 classifier for every accepted image and conditionally invokes the FP16 segmenter; branch-aware operational runtime from three Android devices is archived without claiming field accuracy or iOS support. \\ \hline
...
\rowcolor{RevisionYellow}Final quality audit complete & Section numbering, A4 page geometry, Times New Roman typography, table/figure placement, yellow revision highlighting, terminology, metrics, dates, limitations, and cross-document consistency pass the final rendered audit. \\ \hline
...
\rowcolor{RevisionYellow}Final submission complete & The 5 August initial defense package and the Council-mandated revised package due before 12:00 on 15 August 2026 are both traceable; accepted artifacts are archived. \\ \hline
\end{longtable}
\endgroup
\begin{revisionblock}
...
\end{revisionblock}
\subsubsection{\texorpdfstring{\colorbox{RevisionYellow}{Closure Readiness Review}}{Closure Readiness Review}}
\begin{revisionblock}
...
\end{revisionblock}
\subsubsection{\texorpdfstring{\colorbox{RevisionYellow}{Final Scientific and Reproducibility Audit}}{Final Scientific and Reproducibility Audit}}
\begin{revisionblock}
...
\end{revisionblock}
\subsubsection{\texorpdfstring{\colorbox{RevisionYellow}{Artifact Handover and Project Continuity}}{Artifact Handover and Project Continuity}}
\begin{revisionblock}
...
\end{revisionblock}
\subsubsection{\texorpdfstring{\colorbox{RevisionYellow}{Lessons Learned and Final Evaluation}}{Lessons Learned and Final Evaluation}}
\begin{revisionblock}
