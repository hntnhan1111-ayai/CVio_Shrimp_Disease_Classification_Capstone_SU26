...
def generate_torch_gradcam(run_dir: Path, split_manifest: pd.DataFrame, output_dir: Path, max_per_class: int = 1, progress_enabled: bool = True) -> list[dict[str, Any]]:
    import numpy as np
    import torch
    ...
    if not checkpoint.exists():
        if progress_enabled:
            log_event("Torch XAI skipped because checkpoint is missing.", level="WARNING", run_id=run_dir.name, output_dir=output_dir)
        return [{"run_id": run_dir.name, "backend": "torch", "status": "failed", "reason": "checkpoint_best.pt missing"}]
    if progress_enabled:
        log_event("Starting Torch Grad-CAM generation.", run_id=run_dir.name, output_dir=output_dir, extra={"checkpoint": str(checkpoint)})
    ...
    if target_layer is None:
        if progress_enabled:
            log_event("Torch XAI failed because no Conv2d target layer was found.", level="WARNING", run_id=run_dir.name, output_dir=output_dir)
    ...
    for sample in progress_iter(list(samples.itertuples(index=False)), desc=f"xai torch {run_dir.name}", total=len(samples), enabled=progress_enabled, leave=False):
        try:
            ...
            if progress_enabled:
                log_event("Torch Grad-CAM sample failed.", level="WARNING", run_id=run_dir.name, output_dir=output_dir, extra={"rel_path": sample.rel_path, "reason": repr(exc)})
    ...
    if progress_enabled:
        log_event("Finished Torch Grad-CAM generation.", run_id=run_dir.name, output_dir=output_dir, extra={"rows": len(rows)})
    return rows


def generate_yolo_classification_gradcam(run_dir: Path, yolo_manifest: pd.DataFrame, output_dir: Path, max_per_class: int = 1, progress_enabled: bool = True) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    if progress_enabled:
        log_event("Starting YOLO classification Grad-CAM attempt.", run_id=run_dir.name, output_dir=output_dir)
    try:
        import cv2
        import numpy as np
        import torch
        ...
        if progress_enabled:
            log_event("YOLO XAI dependency check failed.", level="WARNING", run_id=run_dir.name, output_dir=output_dir, extra={"reason": repr(exc)})
    ...
    if not checkpoint.exists():
        if progress_enabled:
            log_event("YOLO XAI skipped because checkpoint is missing.", level="WARNING", run_id=run_dir.name, output_dir=output_dir)
    ...
    try:
        ...
        if target_layer is None:
            if progress_enabled:
                log_event("YOLO XAI failed because no Conv2d target layer was found.", level="WARNING", run_id=run_dir.name, output_dir=output_dir)
        ...
        xai_dir = ensure_dir(output_dir / "figures" / "xai" / run_dir.name)
        for sample in progress_iter(list(samples.itertuples(index=False)), desc=f"xai yolo {run_dir.name}", total=len(samples), enabled=progress_enabled, leave=False):
            try:
                ...
                if progress_enabled:
                    log_event("YOLO Grad-CAM sample failed.", level="WARNING", run_id=run_dir.name, output_dir=output_dir, extra={"rel_path": sample.rel_path, "reason": repr(exc)})
        ...
        if progress_enabled:
            log_event("YOLO Grad-CAM attempt failed.", level="WARNING", run_id=run_dir.name, output_dir=output_dir, extra={"reason": repr(exc)})
        rows.append({"run_id": run_dir.name, "backend": "ultralytics", "status": "failed", "reason": repr(exc)})
    if progress_enabled:
        log_event("Finished YOLO classification Grad-CAM attempt.", run_id=run_dir.name, output_dir=output_dir, extra={"rows": len(rows)})
    return rows


def generate_selected_xai(output_dir: str | Path, metrics_frame: pd.DataFrame, split_manifest: pd.DataFrame | None = None, yolo_manifest: pd.DataFrame | None = None, progress_enabled: bool = True) -> pd.DataFrame:
    output_dir = Path(output_dir)
    rows: list[dict[str, Any]] = []
    if metrics_frame.empty:
        if progress_enabled:
            log_event("Skipping XAI because there are no completed metrics.", output_dir=output_dir)
    ...
    if progress_enabled:
        log_event("Starting selected XAI generation.", output_dir=output_dir, extra={"selected_runs": len(selected)})
    for record in progress_iter(selected.to_dict(orient="records"), desc="selected xai", total=len(selected), enabled=progress_enabled, leave=False):
        ...
        try:
            if backend in {"torchvision", "timm"}:
                if split_manifest is None:
                    ...
                    rows.extend(generate_torch_gradcam(run_dir, split_manifest, output_dir, progress_enabled=progress_enabled))
            elif backend == "ultralytics":
                if yolo_manifest is None:
                    rows.append({"run_id": run_dir.name, "backend": backend, "status": "failed", "reason": "yolo_manifest missing"})
                else:
                    rows.extend(generate_yolo_classification_gradcam(run_dir, yolo_manifest, output_dir, progress_enabled=progress_enabled))
        except Exception as exc:
            if progress_enabled:
                log_event("Selected XAI run failed softly.", level="WARNING", run_id=run_dir.name, output_dir=output_dir, extra={"reason": repr(exc)})
            rows.append({"run_id": run_dir.name, "backend": backend, "status": "failed", "reason": repr(exc)})
    frame = pd.DataFrame(rows)
    save_csv(frame, output_dir / "xai_status.csv")
    if progress_enabled:
        log_event("Selected XAI generation completed.", output_dir=output_dir, extra={"rows": len(frame), "path": str(output_dir / "xai_status.csv")})
    return frame
