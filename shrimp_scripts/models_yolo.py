...
def predict_yolo(best_model, frame: pd.DataFrame, run_id: str, model_name: str, loss_name: str, condition_key: str, randaugment: bool, split_name: str, batch: int):
    torch = _torch()
...
def train_yolo_once(model_name: str, condition: dict[str, Any], yolo_manifest: pd.DataFrame, output_dir: str | Path, resume: bool, smoke_test: bool, batch: int) -> dict[str, Any]:
    ...
    val_metrics, val_predictions, val_true, val_pred = predict_yolo(best_model, val_frame, run_id, model_name, loss_name, condition["condition_key"], condition["randaugment"], "val", batch)
    test_metrics, test_predictions, test_true, test_pred = predict_yolo(best_model, test_frame, run_id, model_name, loss_name, condition["condition_key"], condition["randaugment"], "test", batch)
...
def train_yolo_with_fallback(model_name: str, condition: dict[str, Any], yolo_manifest: pd.DataFrame, output_dir: str | Path, resume: bool = True, smoke_test: bool = False) -> dict[str, Any]:
    run_id = yolo_run_id(model_name, condition)
    errors: list[dict[str, Any]] = []
    for batch in config.YOLO_BATCH_FALLBACKS:
        try:
            return train_yolo_once(model_name, condition, yolo_manifest, output_dir, resume=resume, smoke_test=smoke_test, batch=batch)
        except Exception as exc:
...
def probe_yolo_availability(output_dir: str | Path) -> pd.DataFrame:
    from ultralytics import YOLO
