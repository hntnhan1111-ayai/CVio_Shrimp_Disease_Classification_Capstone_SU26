...
from mobile_corruptions import NOISES, PROTOCOL, apply_corruption, stable_seed
...
def run_pair(python: Path, package: Path, baseline: Path, recsra: Path, data_yaml: Path, output: Path, tag: str) -> tuple[dict, dict]:
    ...
    for label, model in (("baseline", baseline), ("recsra", recsra)):
        ...
        print(f"[EVAL] {tag} | {label} | physical GPU 0", flush=True)
...
def build_samples(top: pd.DataFrame, ds_root: Path, baseline: Path, recsra: Path, package: Path, python: Path, output: Path, seed: int) -> list[dict]:
    ...
    for _, row in top.iterrows():
        ...
        bpred=predict_image(python,package,baseline,corr_path,"0",samples_dir/f"{nid}_baseline_pred.json")
        rpred=predict_image(python,package,recsra,corr_path,"0",samples_dir/f"{nid}_recsra_pred.json")
        panels=[
...
def main() -> None:
    ...
    ap.add_argument("--output",default="/home/drnguyenvinh/notebooks/recsra_mobile10_output_rtx4090")
    ...
    if not gpu_names or "4090" not in gpu_names[0]:
        raise RuntimeError(f"Expected NVIDIA GeForce RTX 4090 as visible GPU 0; found {gpu_names}")
    print(f"[GPU MODE] Sequential single-GPU evaluation on {gpu_names[0]}", flush=True)
    runtime={
        "python":sys.version,
        ...
        "evaluation_only_hardware":gpu_names[0],
        "evaluation_mode":"single-GPU sequential: baseline then RECSRA on physical GPU 0",
    }
    ...
    final_zip=output/"RECSRA_mobile10_RTX4090_final_results.zip"
