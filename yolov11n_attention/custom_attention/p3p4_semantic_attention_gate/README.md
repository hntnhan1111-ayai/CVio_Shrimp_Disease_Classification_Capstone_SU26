# P3P4 Semantic Attention Gate

## Module

- Folder: `p3p4_semantic_attention_gate`
- Python class: `P3P4SemanticAttentionGate`
- Priority: 4
- Model YAML: `model.yaml`
- Notebook: `p3p4_semantic_attention_gate.ipynb`

## Idea

Self-context semantic-style gate as a safe first pass toward guided neck attention.
...
P3/P4 before Segment, single-input safe variant
...
Suppress noisy lateral features and improve mask/detail.

## Risks

True semantic-guided multi-input gate is not used yet; self-context may be weaker.
...
```bash
python custom_attention/_shared/sanity_check.py --model custom_attention/p3p4_semantic_attention_gate/model.yaml --imgsz 640 --device cpu
```
...
Open `p3p4_semantic_attention_gate.ipynb` and run cells only after the sanity check cell passes. The notebook is generated from the leakage-safe baseline and only changes the model YAML, experiment key/name, and output root.
...
mask mAP50-95, healthy FP, BG/WSSV per-class AP, inference speed
