...
- Python class: `CASpatialLowFPGate`
- Priority: 7
- Model YAML: `model.yaml`
- Notebook: `ca_spatial_low_fp_gate.ipynb`

## Idea

Coordinate Attention plus centered spatial gate for localization and FP suppression.
...
Balance position-aware localization with background suppression.

## Risks

Two spatial/position gates can be redundant and reduce small-lesion recall.
...
```bash
python custom_attention/_shared/sanity_check.py --model custom_attention/ca_spatial_low_fp_gate/model.yaml --imgsz 640 --device cpu
```
...
Open `ca_spatial_low_fp_gate.ipynb` and run cells only after the sanity check cell passes. The notebook is generated from the leakage-safe baseline and only changes the model YAML, experiment key/name, and output root.
...
recall, precision, healthy FP, mask AP50-95, latency
