...
- Folder: `ces_lite`
- Python class: `CESLite`
- Priority: 2
- Model YAML: `model.yaml`
- Notebook: `ces_lite.ipynb`

## Idea

Coordinate Attention plus ECA and SimAM-style neuron saliency.
...
Improve lesion localization while staying lightweight.

## Risks

Coordinate and neuron gates may overlap spatially; dynamic shape/export must be checked.
...
```bash
python custom_attention/_shared/sanity_check.py --model custom_attention/ces_lite/model.yaml --imgsz 640 --device cpu
```
...
Open `ces_lite.ipynb` and run cells only after the sanity check cell passes. The notebook is generated from the leakage-safe baseline and only changes the model YAML, experiment key/name, and output root.
...
mask mAP50, mask mAP50-95, recall, per-class AP BG/WSSV, healthy FP
