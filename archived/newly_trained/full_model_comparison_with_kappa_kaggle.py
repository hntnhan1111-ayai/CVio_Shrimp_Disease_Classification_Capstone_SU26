...
# %%
print("Running Kaggle version. Input data is read from /kaggle/input; outputs are written to /kaggle/working.")
...
OUTPUT_DIR = Path("/kaggle/working/full_lightweight_model_comparison")
...
def train_yolo_model(model_name: str) -> dict:
    ...
    print_gpu_status(f"{model_name} before YOLO train")
    yolo.train(
    ...
    print_gpu_status(f"{model_name} after YOLO train")
    train_time = time.time() - train_start
    ...
    print_gpu_status(f"{model_name} before YOLO eval")
    ...
    print_gpu_status(f"{model_name} after YOLO eval")
    inf_time = test_metrics["elapsed"]
...
def rebuild_timm_from_checkpoint(checkpoint_path: str):
    ...
    if checkpoint.get("backend_source") == "torchvision":
        model = TorchvisionShrimpXNet(checkpoint["display_name"], pretrained=False)
    else:
