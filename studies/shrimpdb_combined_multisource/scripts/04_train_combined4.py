#!/usr/bin/env python3
"""Train YOLO26m-cls with ASL-LDAM + SimAM-DCFR on Combined-4."""
...
DATASET_CONFIG = Path("configs/datasets/combined4.yaml")
...
def run(
    ...
    run_name = dataset.get("run_name", "yolo26m_cls__asl_ldam_simam_dcfr__combined4__seed42")
    ...
    try:
        ...
        write_status(actual_run_dir, "ok", experiment="combined4", seed=seed)
        LOGGER.info("Combined-4 training completed: %s", actual_run_dir)
        return 0
    except Exception as exc:
        write_status(run_dir, "error", error=str(exc))
        LOGGER.error("Combined-4 training failed: %s", exc)
        return 1


def main() -> int:
    parser = argparse.ArgumentParser(description="Train Combined-4 experiment")
