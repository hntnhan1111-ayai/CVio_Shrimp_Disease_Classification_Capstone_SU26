...
> **No raw dataset or trained `.pt` checkpoints are included.** The TFLite files in `export/`
> are the only committed model artifacts.
...
export/               TFLite model files (FP32, FP16) and sanity checks
artifacts/            Manifests, tables, figures, evaluation results, metadata
docs/                 9 documentation files (this set)
tests/                Lightweight tests (no full dataset required)
model_registry/       Checkpoint hashes and class mappings
...
|---|---|---:|---:|---:|---:|
| best.pt (validation-selected) | <= 15 (early stop) | 83.18% | 83.85% | 82.18% | 33.46% |
...
| Figure | Description |
|---|---|
| `artifacts/figures/matched_shrimpdb_test_domain_comparison.png` | Matched-domain comparison bar chart |

Additional figures are generated by `06_generate_academic_artifacts.py` and stored in
`artifacts/figures/`.
